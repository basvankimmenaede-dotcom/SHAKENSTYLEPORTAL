...
function LoginForm() {
  ...
  async function forgotPassword() {
    ...
    try {
      ...
      if (resetError) {
        setError('De resetmail kon niet worden verstuurd. Probeer het opnieuw.');
        return;
      }
    ...
    <main className="loginPage">
      <section className="loginCard">
        ...
        <form onSubmit={submit}>
          ...
          <button className="button orange" type="submit" disabled={loading || resetLoading} style={{ width: '100%' }}>
            {loading ? 'Inloggen...' : 'Inloggen'}
          </button>
        </form>

        <button type="button" className="textButton" onClick={forgotPassword} disabled={loading || resetLoading}>
          {resetLoading ? 'Resetmail versturen...' : 'Wachtwoord vergeten?'}
        </button>
      </section>
    </main>
  );
